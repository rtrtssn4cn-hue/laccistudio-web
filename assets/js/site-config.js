/* =========================================================================
   LACCI STUDIO — YOUR CONTACT INFO (edit this one file)
   -------------------------------------------------------------------------
   Change the text between the quote marks " ".
   Whatever you type here updates every page automatically.
   Leave a value empty ("") to hide it (e.g. hide the phone or a social icon).
   ========================================================================= */

window.LACCI_CONFIG = {

  // --- Contact ---
  email: "info@laccistudio.com",     // your business email
  phone: "",                        // e.g. "(555) 123-4567"  — leave "" to hide

  // --- Social links (paste the full web address, or leave "" to hide the icon) ---
  instagram: "",                    // e.g. "https://instagram.com/laccistudio"
  facebook:  "",                    // e.g. "https://facebook.com/laccistudio"
  etsy:      ""                     // e.g. "https://laccistudio.etsy.com"

};
